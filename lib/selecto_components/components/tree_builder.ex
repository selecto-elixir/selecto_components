defmodule SelectoComponents.Components.TreeBuilder do
      ...
      <div class="">
        ...
        >
          <div class="min-w-0" style="color: var(--sc-text-primary);">
            Available Filter Columns. Double Click or Drag to build area.
            <div class="mt-2 flex items-center gap-1">
                  ...
                  <div class="space-y-2">
                    ...
                    >
                      <input
                        ...
                        type="checkbox"
                        value={to_string(type_filter.key)}
              ...
              <button
                id={"clear-filter-#{@id}"}
                class={[Theme.slot(@theme, :button_danger), "h-7", "w-7"]}
                type="button"
                title="Clear filter"
              >
                ×
          ...
          <div class="min-w-0" style="color: var(--sc-text-primary);">
            Build Area. All top level filters are AND'd together and AND'd with the required filters from the domain.
          </div>

          <div
            class={
              Theme.slot(@theme, :panel) <>
                " flex h-96 min-h-0 flex-col gap-2 overflow-auto p-2 xl:h-[32rem]"
            }
            style="background: var(--sc-surface-bg-alt);"
            id={"available-items-#{@id}"}
          >
            <div
            ...
            <div
              :for={{{id, name, type_meta}, idx} <- Enum.with_index(@available)}
            ...
            >
              <div class="flex items-start gap-2">
                ...
                <.choice_source_indicator type={type_meta} />
          ...
          <div
            class={
              Theme.slot(@theme, :panel) <>
                " drop-zone grid h-96 min-h-0 grid-cols-1 gap-2 overflow-auto p-2 xl:h-[32rem]"
            }
  ...
  defp render_area(assigns) do
    ...
    ~H"""
    <div
      class="rounded-xl border p-1 pb-8 drop-zone"
      ...
      <%= for {{uuid, s_section, config} = s, index} <-
        ...
        >
          <%= case {uuid, s_section, config} do %>
            ...
            <% {uuid, section, fv} -> %>
              <div
                class="rounded-md border p-2 pl-6"
