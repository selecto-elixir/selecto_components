defmodule SelectoComponents.Performance.VirtualScroll do
  ...
  def handle_event(
        "handle_scroll",
        %{"scrollTop" => scroll_top, "scrollHeight" => scroll_height},
        socket
      ) do
    ...
    # ~60fps
    if current_time - socket.assigns.last_scroll_time > 16 do
      socket =
        socket
        |> assign(
  ...
  defp maybe_load_more_data(socket, scroll_top, scroll_height) do
    ...
    threshold = scroll_height - viewport_height - 200
  ...
  defp row_style(_index, row_height, _row_heights, false) do
