defmodule SelectoComponents.UI do
  ...
  def build_form_config(view_config) do
    Ecto.Changeset.cast({%{}, %{}}, view_config, [])
    |> Phoenix.Component.to_form(as: "view_config")
  end
  ...
  def validate_view_config_fields(selecto, view_config) do
    ...
    errors =
      validate_field_list(selecto, Map.get(view_config, :selected, []), "selected") ++ errors

    # Validate group by fields
    errors =
      validate_field_list(selecto, Map.get(view_config, :group_by, []), "group_by") ++ errors
  ...
  defp migrate_selected_fields(view_config) do
    case Map.get(view_config, :selected) do
      ...
      selected_fields when is_list(selected_fields) ->
        migrated_fields =
          Enum.map(selected_fields, &ParameterizedFieldBuilder.migrate_field_reference/1)
  ...
  defp migrate_group_by_fields(view_config) do
    case Map.get(view_config, :group_by) do
      ...
      group_by_fields when is_list(group_by_fields) ->
        migrated_fields =
          Enum.map(group_by_fields, &ParameterizedFieldBuilder.migrate_field_reference/1)
  ...
  defp migrate_single_filter(filter) when is_map(filter) do
    case Map.get(filter, :field) do
      ...
      field_ref ->
        Map.put(filter, :field, ParameterizedFieldBuilder.migrate_field_reference(field_ref))
    end
  end
