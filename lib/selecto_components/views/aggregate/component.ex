defmodule SelectoComponents.Views.Aggregate.Component do
  ...
  defp tree_table(%{subs: {subs, _}} = assigns) do
    ...
    groups =
      assigns.payload
      |> IO.inspect(label: "Current Payload for Groups")
      |> Enum.reduce(
        [],
        fn {i, {_, {:group_by, _, coldef}}, v, _}, acc ->
          ...
          prefil =
            ...
            |> List.first()
          IO.inspect(prefil, label: "Previous Filters")
          IO.inspect(coldef, label: "Column Definition")
          IO.inspect(v, label: "Current Value")
          ...
          newfil =
            case {coldef, v} do
              ...
              {coldef, {_, filt}} when is_map(coldef) ->
                IO.inspect(coldef, label: "Column Definition for Rollup Row")
                filter_key = Map.get(coldef, :group_by_filter, Map.get(coldef, :colid, Map.get(coldef, :name)))
                IO.puts("Filter key for rollup row: #{inspect(filter_key)}")
              ...
              {coldef, _} when is_map(coldef) ->
                filter_key = Map.get(coldef, :group_by_filter, Map.get(coldef, :colid, Map.get(coldef, :name)))
                IO.puts("Filter key for regular row: #{inspect(filter_key)}")
                if filter_key != nil do
